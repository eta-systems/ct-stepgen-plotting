...
@brief     Example to trace and plot a simple resistor curve with 100 data points
           with an averaging (smoothing) through multiple measurements
...
#%%
# i curves 
for i in range(0, N_OVERSAMPLES):
    # with k samples
    for k in range(0, N_POINTS):
        ...
        val['source']['voltage'][k] += (vread/N_OVERSAMPLES)
        val['source']['current'][k] += (iread/N_OVERSAMPLES)
...
plt.title(r'current-voltage curve of a 1$k\Omega$ resistor (3x average)')
plt.grid()
